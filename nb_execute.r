library("e1071")
d = read.table("out",stringsAsFactors = FALSE)
for(i in 1:dim(d)[1])
{
result = tryCatch(
	{
 data <- read.csv(paste("./Data/Processed/Goalkeepers/",d[i,1],sep=""))
...
nb_model<-naiveBayes(as.factor(y) ~ .,data=train_data)

pred <- predict(nb_model, features[no_rows,])
